...
5. **Phase 5 — Post-Game Analytics** (`app/core/heatmap.py`): *(NEW)*
...
| `HF_TOKEN` | ⚠️ Recommended | HuggingFace token — enables Kokoro-82M TTS (Tier 1). Free at [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens) |
...
```
*Wait for the service to log `🚀 Starting Inference Server` and `Heatmap module loaded ✓`. YOLO weights are loaded into memory on first request.*
