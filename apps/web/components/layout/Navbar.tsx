...
export function Navbar() {
  ...
  return (
    <nav className="w-full border-b border-border bg-background/80 backdrop-blur-md sticky top-0 z-50 shadow-2xl">
      {/* Content Layer */}
      <div className="relative z-10 flex items-center justify-between py-4 px-6 md:px-8 max-w-[1440px] mx-auto">
        <Link 
          href="/" 
          className="flex items-center gap-3 transition-opacity duration-200 hover:opacity-80 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded-sm group" 
          aria-label="Go to homepage"
        >
          {/* Custom Logo Image replaces the generic 'M' block */}
          <div className="relative size-8 shrink-0 overflow-hidden transform group-hover:scale-105 transition-transform duration-300">
            <Image 
          ...
          <div className="flex items-baseline gap-1">
            <span className="font-display tracking-[0.12em] text-[18px] md:text-[20px] text-foreground drop-shadow-md">MATCHA</span>
            <span className="font-display tracking-[0.12em] ml-1 text-[18px] md:text-[20px] text-primary drop-shadow-[0_0_8px_rgba(var(--color-primary),0.5)]">AI</span>
          </div>
          <div className="hidden sm:block w-px h-4 mx-2 bg-border flex-shrink-0" />
          <span className="hidden sm:inline-block font-mono text-[9px] text-muted-foreground uppercase tracking-[0.14em]">DTU EDITION</span>
        </Link>
