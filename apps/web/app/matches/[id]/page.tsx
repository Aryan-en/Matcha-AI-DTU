...
function IntensityChart({ scores, duration }: { scores: EmotionScore[]; duration: number }) {
  ...
  return (
    <div className="w-full overflow-hidden bg-[#141414] border border-zinc-800 p-4">
      <div className="flex items-center gap-2 mb-3">
        <TrendingUp className="w-4 h-4 text-emerald-500" />
        <span className="text-xs font-semibold text-zinc-300 uppercase tracking-wide">Match Intensity</span>
        <span className="ml-auto text-xs text-zinc-600">{scores.length} data points</span>
      </div>
  ...
  return (
    <div className="bg-[#141414] border border-zinc-800 p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-zinc-300 uppercase tracking-wide">Events Timeline</span>
        <span className="ml-auto text-xs text-zinc-600">{events.length} events</span>
      </div>
      <div className="relative h-8 bg-zinc-800 overflow-visible">
        {events.map((ev) => {
      ...
      <div className="flex flex-wrap gap-3 mt-3">
        {Object.entries(EVENT_CONFIG).map(([k, v]) => (
          <div key={k} className="flex items-center gap-1 text-xs text-zinc-600">
            <div className={`w-2 h-2 rounded-full ${v.color.replace("text-", "bg-")}`} />
            {v.label}
          </div>
        ))}
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="bg-zinc-900 border border-zinc-700 rounded-2xl p-6 max-w-sm w-full mx-4 shadow-2xl">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <p className="font-semibold text-white">Delete Analysis</p>
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={onConfirm}
            disabled={loading}
            className="flex-1 py-2 rounded-lg bg-red-500 hover:bg-red-600 disabled:opacity-50 text-white text-sm font-medium transition-colors flex items-center justify-center gap-2"
          >
...
export default function MatchDetailPage() {
  ...
  // Canvas tracking is now handled inside VideoPlayer component.

  const seekTo = (t: number) => { videoSeekRef.current(t); };

  const playHighlight = (h: Highlight) => {
    setActiveHighlight(h);
    setTimeout(() => seekTo(h.startTime), 100);
  };
  ...
  const handleReanalyze = async () => {
    ...
    try {
      ...
      // Optimistically update status so the progress bar appears immediately
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center text-zinc-400">
        <Loader2 className="w-6 h-6 animate-spin mr-2" /> Loading matchâ€¦
      </div>
    );
  }
  if (!match) {
  ...
  const byType = match.events.reduce<Record<string, number>>((acc, e) => {
    acc[e.type] = (acc[e.type] ?? 0) + 1; return acc;
  }, {});
  const topScore = match.events.length ? Math.max(...match.events.map(e => e.finalScore)) : 0;
  const avgConf  = match.events.length ? match.events.reduce((s, e) => s + e.confidence, 0) / match.events.length : 0;
  const top5Moments = [...match.events].sort((a, b) => b.finalScore - a.finalScore).slice(0, 5);
  const goalCount = byType["GOAL"] ?? 0;
  const saveCount = byType["SAVE"] ?? 0;

  // Live intensity at current video time (nearest motion window)
  const liveIntensity = (() => {
    if (!match.emotionScores.length) return 0;
    const nearest = match.emotionScores.reduce((prev, cur) =>
      Math.abs(cur.timestamp - currentTime) < Math.abs(prev.timestamp - currentTime) ? cur : prev
    );
    return nearest.motionScore;
  })();

  const allEventTypes = Array.from(new Set(match.events.map(e => e.type)));
  const filteredEvents = eventTypeFilter === "ALL"
    ? match.events
    : match.events.filter(e => e.type === eventTypeFilter);

  // Live events sorted by timestamp
  const sortedLive = [...liveEvents].sort((a, b) => b.timestamp - a.timestamp);
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      {/* Nav */}
      <nav className="border-b border-zinc-800 px-6 py-4 flex items-center gap-4 bg-[#0f0f0f]">
        <Link href="/" className="flex items-center gap-2 text-zinc-400 hover:text-white transition-colors text-sm uppercase tracking-wide">
          <ArrowLeft className="w-4 h-4" /> Back
        </Link>
        <div className="h-4 w-px bg-zinc-700" />
        <span className="font-mono text-sm text-zinc-500 truncate max-w-xs">{match.id}</span>
        <span className={`ml-auto inline-flex items-center gap-1 text-xs px-3 py-1.5 border font-medium uppercase tracking-wide
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-xs text-zinc-500 hover:text-emerald-400 border border-zinc-700 hover:border-emerald-500/50 px-4 py-1.5 transition-all disabled:opacity-40 uppercase tracking-wide"
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-xs text-zinc-500 hover:text-red-400 border border-zinc-700 hover:border-red-500/50 px-4 py-1.5 transition-all uppercase tracking-wide"
        >
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        {/* No-tracking banner for older videos — removed: tracking is now live in-browser */}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            <div key={stat.label} className="bg-[#141414] border border-zinc-800 p-4 hover:border-emerald-500/40 transition-all card-flat">
        ...
        {match.summary && (
          <div className="bg-[#141414] border border-zinc-800 p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-semibold text-zinc-200 font-heading uppercase tracking-wide">AI Match Analysis</span>
              <span className="ml-auto text-[10px] text-zinc-500 border border-zinc-700 px-2 py-0.5 uppercase tracking-wider">Gemini 2.0 Flash</span>
            </div>
            <p className="text-sm text-zinc-400 leading-relaxed whitespace-pre-line">{match.summary}</p>
          </div>
        )}
        ...
        {top5Moments.length > 0 && (
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-semibold text-zinc-200 font-heading uppercase tracking-wide">Top 5 Moments</span>
              <span className="text-xs text-zinc-600 ml-1">by context score · click to seek</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
              {top5Moments.map((ev, i) => {
                ...
                const rankStyle = rank === 1 ? "border-amber-400/50 bg-amber-400/5 hover:border-amber-400"
                                : rank === 2 ? "border-zinc-500/50 bg-zinc-500/5 hover:border-zinc-400"
                                : rank === 3 ? "border-amber-700/50 bg-amber-700/5 hover:border-amber-600"
                                : "border-zinc-800 bg-zinc-900/50 hover:border-zinc-700";
                const rankColor = rank === 1 ? "text-amber-400" : rank === 2 ? "text-zinc-300" : rank === 3 ? "text-amber-600" : "text-zinc-500";
                return (
                  <button
                    key={ev.id}
                    onClick={() => seekTo(ev.timestamp)}
                    className={`relative text-left border p-3 transition-all hover:translate-y-[-2px] group ${rankStyle}`}
                  >
                    <div className={`absolute -top-2 -left-2 w-5 h-5 bg-black border flex items-center justify-center text-[10px] font-black ${rankColor} border-current`}>
                      {rank}
                    </div>
                    <span className={`inline-flex items-center gap-1 text-xs px-1.5 py-0.5 border mb-2 ${cfg.bg} ${cfg.border} ${cfg.color}`}>
                      {cfg.icon} {cfg.label}
                    </span>
                    <p className="font-mono text-xs text-zinc-400 mb-1">{fmt(ev.timestamp)}</p>
                    ...
                    {ev.commentary && (
                      <p className="text-[10px] text-zinc-600 line-clamp-2 group-hover:text-zinc-500 transition-colors">{ev.commentary}</p>
                    )}
                  </button>
                );
              })}
        ...
        <div className="grid lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-3 space-y-4">
            {match.uploadUrl && (
              <div className="space-y-3">
                {/* ── Custom Video Player (tracking, highlights, TTS) ──── */}
                <VideoPlayer
                  src={match.uploadUrl}
                  events={match.events}
                ...
                {/* Scoreboard HUD (below video) */}
                <div className="bg-[#0f0f0f] border border-zinc-800 px-4 py-3 flex items-center gap-6">
                  <div className="flex items-center gap-1.5">
                    <div className={`w-2 h-2 ${
                      match.status === "COMPLETED" ? "bg-emerald-400" :
                      match.status === "PROCESSING" ? "bg-blue-400 animate-pulse" : "bg-zinc-500"
                    }`} />
                  ...
                  <div className="flex items-center gap-4 flex-1">
                    <div className="text-center">
                      <p className="text-[9px] text-zinc-600 uppercase tracking-wide">Goals</p>
                      <p className="text-xl font-black text-white leading-none">{goalCount}</p>
                    </div>
                    <div className="text-zinc-700 text-sm">•</div>
                    <div className="text-center">
                      <p className="text-[9px] text-zinc-600 uppercase tracking-wide">Saves</p>
                      <p className="text-xl font-black text-white leading-none">{saveCount}</p>
                    </div>
                    <div className="flex-1 space-y-1 ml-4">
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-[9px] text-zinc-600 uppercase tracking-wide">Intensity</span>
                  ...
                  <span className="font-mono text-xs text-zinc-500">{fmt(currentTime)}</span>
                </div>
              </div>
            )}
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {(match.status === "PROCESSING" || (match.status === "UPLOADED" && liveEvents.length > 0)) && (
              <div className="bg-[#0a0a0a] border border-blue-500/30 overflow-hidden">
                <div className="flex items-center gap-2 px-4 py-2.5 border-b border-zinc-800 bg-blue-500/5">
                  <Radio className="w-3.5 h-3.5 text-blue-400 animate-pulse" />
                  <span className="text-xs font-bold text-blue-300 tracking-widest uppercase">Live Detection</span>
                  <span className="ml-auto text-[10px] text-zinc-500">{liveEvents.length} events found</span>
                </div>
                <div className="max-h-52 overflow-y-auto space-y-px">
                  {liveEvents.length === 0 && (
                    <div className="flex items-center gap-2 px-4 py-3 text-xs text-zinc-600">
                      <Loader2 className="w-3.5 h-3.5 animate-spin" /> Scanning frames…
                    </div>
                  )}
                  {sortedLive.map((ev, i) => {
                    const cfg = EVENT_CONFIG[ev.type] ?? DEFAULT_EVT;
                    return (
                      <button
                        key={i}
                        onClick={() => seekTo(ev.timestamp)}
                        className={`w-full flex items-center gap-3 px-4 py-2 text-left hover:bg-zinc-900 transition-colors ${i === 0 ? 'animate-pulse' : ''}`}
                      >
                        <span className={`inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 border shrink-0 ${cfg.bg} ${cfg.border} ${cfg.color}`}>
                          {cfg.icon} {cfg.label}
                        </span>
                        <span className="font-mono text-[10px] text-zinc-400">{fmt(ev.timestamp)}</span>
                        <span className={`ml-auto font-bold text-[10px] font-mono ${
                          ev.finalScore >= 7.5 ? 'text-emerald-400' : ev.finalScore >= 5 ? 'text-amber-400' : 'text-zinc-600'
            ...
            {/* Tabs */}
            <div className="flex bg-[#0f0f0f] border border-zinc-800 p-1 gap-1">
              {(["highlights", "events"] as const).map((tab) => (
                <button
                  key={tab}
                  onClick={() => setActiveTab(tab)}
                  className={`flex-1 text-sm py-2 font-medium transition-all uppercase tracking-wide
                    ${activeTab === tab
            ...
            {/* â”€â”€ Highlights tab â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
            {activeTab === "highlights" && (
              <div className="space-y-3 max-h-[600px] overflow-y-auto pr-1">
                {match.highlightReelUrl && (
                  <div className="mb-4 p-4 bg-emerald-900/20 border border-emerald-500/30">
                    <h3 className="text-sm font-bold text-emerald-400 mb-2 flex items-center gap-2 font-heading uppercase tracking-wide">
                      <Film className="w-4 h-4" /> Full Highlight Reel
                    </h3>
                    ...
                    <a
                      ...
                      className="inline-flex items-center gap-2 text-sm font-semibold text-black bg-emerald-500 hover:bg-emerald-400 px-4 py-2 transition-all uppercase tracking-wide"
                    >
                      <Play className="w-4 h-4" /> Watch Reel
                    </a>
                  </div>
                )}
                {!match.highlights.length && (
                  <div className="text-center text-zinc-600 text-sm py-12 border border-dashed border-zinc-700 bg-zinc-900/50">
                ...
                {match.highlights.map((h, i) => {
                  ...
                  return (
                    <div key={h.id} className={`border p-4 transition-all ${isActive ? "border-emerald-500/50 bg-emerald-500/5" : "border-zinc-800 bg-[#141414] hover:border-zinc-600"}`}>
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <div className="flex items-center gap-2">
                          <span className="w-6 h-6 bg-zinc-800 flex items-center justify-center text-xs text-zinc-500 font-bold shrink-0">{i + 1}</span>
                          {h.eventType && (
                      ...
                      <div className="h-1 bg-zinc-800 mb-3 overflow-hidden">
                        <div
                      ...
                      <div className="flex items-center gap-3 text-xs text-zinc-500 mb-2">
                        ...
                        <span className="font-mono">{fmt(h.startTime)} â†’ {fmt(h.endTime)}</span>
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="flex items-center gap-1.5 text-xs font-medium text-emerald-400 hover:text-emerald-300 border border-emerald-500/30 hover:border-emerald-500 px-3 py-1.5 transition-all bg-emerald-500/5 hover:bg-emerald-500/10 uppercase tracking-wide"
                        >
                        ...
                        {h.videoUrl && (
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center gap-1.5 text-xs font-medium text-blue-400 hover:text-blue-300 border border-blue-500/30 hover:border-blue-500 px-3 py-1.5 transition-all bg-blue-500/5 hover:bg-blue-500/10 uppercase tracking-wide"
                          >
                            <Film className="w-3.5 h-3.5" /> View Generated Clip
                          </a>
                        )}
                      </div>
                    </div>
            ...
            {/* â”€â”€ Events tab â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
            {activeTab === "events" && (
              <div className="space-y-3">
                {/* Event type filter */}
                {allEventTypes.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    <button
                      ...
                      className={`text-xs px-3 py-1.5 border transition-all uppercase tracking-wide ${
                        eventTypeFilter === "ALL"
                    ...
                    {allEventTypes.map((type) => {
                      ...
                      return (
                        <button
                          ...
                          className={`inline-flex items-center gap-1 text-xs px-3 py-1.5 border transition-all uppercase tracking-wide ${
                  ...
                  {filteredEvents.map((ev) => {
                    ...
                    return (
                      <button
                        key={ev.id}
                        onClick={() => seekTo(ev.timestamp)}
                        className="w-full text-left border border-zinc-800 hover:border-zinc-600 bg-[#141414] hover:bg-zinc-900 p-3 transition-all group"
                      >
                        <div className="flex items-center gap-3">
                          <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 border shrink-0 ${cfg.bg} ${cfg.border} ${cfg.color}`}>
                            {cfg.icon} {cfg.label}
                          </span>
                          <span className="text-xs text-zinc-500 font-mono shrink-0">{fmt(ev.timestamp)}</span>
                          <div className="flex-1 min-w-0">
                            {ev.commentary && (
                              <p className="text-xs text-zinc-600 truncate group-hover:text-zinc-400 transition-colors">
                                {ev.commentary}
                              </p>
                            )}
                          </div>
                          <div className="shrink-0 flex items-center gap-2">
                            ...
                            <span className="text-xs text-zinc-700">{(ev.confidence * 100).toFixed(0)}%</span>
                            <ScoreBadge score={ev.finalScore} />
                          </div>
                        </div>
                      </button>
